...
begin
 ...
 {$ENDIF}
  ...
  ' -K - Build Knowledge Base File for IDR (used by -U with *)'#13#10
  );
end ;
...
begin
  ...
  try
    ...
    ModuleInfo^.Filename := ExtractFileName(FN);
    ...
    try
      ...
      if U<>Nil then begin
        {phroyt(}
        ModuleInfo^.Name := String(U.UnitName);
        {)phroyt}
        U.Show;
        if Queries<>[] then
          QueryUnit(U,Queries);
